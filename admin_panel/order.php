...
<!--  In THis Commet code block with @@@@@@@@@@@@@@ Icon -->
		...
		$pages_query = mysql_query("SELECT COUNT(`id`) FROM `order_pro`");
		...
		$start = ($page - 1) * $per_page;
				...
				$sql = mysql_query("SELECT * FROM `products` INNER JOIN `order_pro` ON `products`.`id` = `order_pro`.`pro_id` LIMIT $start , $per_page");
					...
					<a href="order.php?del=<?php echo $read['id'];?>" title="Delete" class="icon-2 info-tooltip"></a>
		
					<a href="order.php?approve_id=<?php echo $read['id'];?>" title="Aprove Order" class="icon-4 info-tooltip"></a>
